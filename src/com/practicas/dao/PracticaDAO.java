...
public class PracticaDAO {
    ...
    public void insertar(Practica p) throws SQLException {
        if (p == null) throw new SQLException("La practica es null");
        String sql = """
            INSERT INTO PRACTICA
            (NOMBRE, DESCRIPCION, NUMERO_PRACTICA, HORAS_REGLAMENTARIAS,
             FECHA_INICIO, FECHA_FIN, ESTADO, TIPO_PRACTICA, FECHA_CREACION)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """;
        ...
        ps.setDate(5, p.getFechaInicio() != null ? java.sql.Date.valueOf(p.getFechaInicio()) : null);
        ps.setDate(6, p.getFechaFin() != null ? java.sql.Date.valueOf(p.getFechaFin()) : null);
        ps.setString(7, p.getEstado() != null ? p.getEstado().name() : null);
        ps.setString(8, p.getTipoPractica());
        ps.setTimestamp(9, p.getFechaCreacion() != null ? Timestamp.valueOf(p.getFechaCreacion()) : null);
        ps.executeUpdate();
        ps.close();
    }
    ...
    public List<Practica> listar() throws SQLException {
        ...
        String sql = "SELECT ID_PRACTICA, NOMBRE, DESCRIPCION, NUMERO_PRACTICA, HORAS_REGLAMENTARIAS, FECHA_INICIO, FECHA_FIN, ESTADO, TIPO_PRACTICA, FECHA_CREACION FROM PRACTICA";
        PreparedStatement ps = conn.prepareStatement(sql);
        ...
        while (rs.next()) {
            ...
            if (estado != null) p.setEstado(Practica.EstadoPractica.valueOf(estado));
    ...
    public void actualizar(Practica p) throws SQLException {
        String sql = """
            UPDATE PRACTICA
            SET NOMBRE = ?, DESCRIPCION = ?, NUMERO_PRACTICA = ?, HORAS_REGLAMENTARIAS = ?,
                FECHA_INICIO = ?, FECHA_FIN = ?, ESTADO = ?, TIPO_PRACTICA = ?
            WHERE ID_PRACTICA = ?
        """;
    ...
    public void eliminar(int idPractica) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement("DELETE FROM PRACTICA WHERE ID_PRACTICA = ?")) {
